...
- Enable Bedrock prompt caching for prefixes longer than 1,024 tokens
...
**Implementation:**
- Add a batching window (configurable, default 5 minutes) to the demand sensing agent
- Accumulate signals in a Lambda-internal buffer or short SQS delay queue
...
**Implementation:**
- Before calling Bedrock, check if the signal falls within 1 standard deviation of the historical norm for that SKU/location/time
- If yes: no adjustment needed, suppress without invoking the model
...
**Optimization:**
- Review Neptune CloudWatch metrics weekly
- If max NCUs never exceed 4, set the max to 5 (small buffer)
- If queries are primarily read-heavy, consider read replicas instead of scaling the writer
...
**Optimization:**
- If the vector collection is primarily queried during business hours (6 AM - 8 PM), consider time-based scaling policies
- Archive decisions older than 90 days to S3 (cold storage) and remove from the vector index
...
**Optimization:**
- Use [AWS Lambda Power Tuning](https://github.com/alexcasalboni/aws-lambda-power-tuning) to find optimal memory for each agent
- Most agents run well at 512MB. Disruption response (which processes larger context windows) may benefit from 1024MB
...
- **One avoided expedited air shipment** (ocean-to-air escalation avoided): $200K+ saved
